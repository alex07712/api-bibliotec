...
class PrestamoController {
    ...
    static crearPrestamo = async (req, res) => {
        try {
            const { libroId, usuarioId, fechaDevolucionEstimada } = req.body;
    ...
    // Obtener TODOS los préstamos (solo para admin)
    static obtenerTodosPrestamos = async (req, res) => {
        try {
            const prestamos = await Prestamo.find()
                .populate("usuario", "nombre nomuser correo") // selecciona campos del usuario
